...
TEST_F(QualityServiceTest, GradeBoundaries) {
    ...
    EXPECT_EQ(service.calculateGrade(89), "B");
...
TEST_F(QualityServiceTest, BookingRules) {
    ...
    EXPECT_TRUE(service.canBookSeats({3, true, 150, false}));
}
...
TEST_F(QualityServiceTest, UsernameLength) {
...
TEST_F(QualityServiceTest, SensorAverageNegative) {
    EXPECT_DOUBLE_EQ(service.calculateSensorAverage({-1}), -1.0);
}
...
TEST_F(QualityServiceTest, SensorHealthOK) {
